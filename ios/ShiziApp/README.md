...
同时它会让 WKWebView 在 App 内执行一次原生 smoke：验证 7294 个可练字、开发入口隔离、本地笔画与 1100+ 条离线字源读取、页面导航、手写、防滚动、回笔、双指透视、字库专项、永久跳过和备份恢复。拍字断言会把一张真实可解码 PNG 送入裁剪压缩流程，确认不超过 64KiB 的 WebP/JPEG 缩略图与大图可读、Vision 请求实际完成、候选必须点选、失败可回退手输、生活来源落盘、单张/总量预算以及备份恢复；这只验证技术链路，不替代真机相机、照片方向和 OCR 质量验收。PointerEvent 手写还会强制检查手指等宽、Pencil 真实压力、零位移墨点、取消/中断不落墨，以及实时、恢复与分享走同一忠实渲染路径。习字链会真实提交「点拨笔画＋用户墨迹」完整字格，验证二选一判断、1.8 秒反馈、Again 事件、原子“重盖”、两步不会写教学、稳定字键 session v3、再练队列、跳过撤销以及退出后原位续练；结算断言覆盖月度节奏、三档互斥难度反馈、第一次没写稳的字再练入口和本地分享图片的尺寸、桥接与隐私边界。原生探针还会回放旧质量标记迁移、当前语境停用与撤销、通知未知/拒绝/授权/关闭的完整序列，并确认备份授权不能冒充当前设备状态。字卡断言覆盖 v2 真迹矢量重绘、3:4 / 1:1 尺寸、每日一次提示、关闭偏好、相册保存与系统分享消息，并确认旧版 v1 栅格墨迹不会带着字格进入字卡；备份断言覆盖 session v3、FSRS review log、教学说明标记、本地漏斗、补帖活动、拍字记录、跳过状态、字卡偏好和限量最近墨迹。
...
脚本会构建、安装、运行两次 App，从真机 app data container 拉回 smoke JSON，并验证页面导航、离线笔画、字库输入/拍字、跳过、备份/恢复、习字/退出流程、软件键盘触发后的 viewport/inset/按钮可达性，以及 `localStorage` 跨进程重启持久化。开发入口加 `DEV_MODE=1`。真机书写手感、旋转、Files/iCloud 选择器和飞行模式仍需按 `DEVICE_QA.md` 人工检查。

第一次真机安装后重点测：

- 首页、习字页、字库、我的、写得不稳的字都能打开。
- 手指在田字格书写时页面不滚动，笔迹不断裂。
- 点拨、完整「你写的」字格、对错判断、自动盖章、1.8 秒反馈和「重盖」流程正常。
- 「不会写」完成「第 1 步：描红 → 第 2 步：自己写 → 本组稍后不看提示再写一次」，描红本身不算掌握。
- 双指按住田字格可透视提示，松开任一指恢复，透视期间不会留下墨迹。
- 在字库输入一个或多个字并点「练这个字 / 练这几个字」后进入专项；未完成的普通组退出专项后原样恢复，专项不单独完成每日组。
- 在字库「拍字」中分别拍路牌、书页和菜单：每次只显示候选，不自动开始练习；点选目标并确认后进入专项，详情显示本地来源照片。题库外汉字在开始时建立个人字卡，没有本地笔顺时直接使用可自行判断的降级流程。
- 点「跳过」后该字不再自动安排，也立即退出未来通知；旧通知点击不会将它重新插入练习。短暂撤销可恢复原字、墨迹和提示，字库「练这个字」可恢复自动安排；若与挂起普通组重叠，普通组按原来源补位或保持未完成。
- 独立写出一个字后确认轻提示不打断换题；从提示或单字详情进入字卡，分别保存 3:4 / 1:1 PNG 到相册并用微信发给自己，检查真实笔迹、宣纸色、字/拼音、干支日期、小章和落款的清晰度与裁切。
- 返回按钮与左缘侧滑都直接保存并回首页，保留当前墨迹、教学阶段和再练队列。
- 完成第一组后「今日已拾」不回退，加练只累加今日字数。
- 「我的」印历用「盖章 N天」统计所选月份的落印日期，用「累计 N天」显示累计练习日，读屏完整说明为「累计练习 N 天」；累计值沿用升级前的历史打开日，升级后只在完整练完一组的日期增加，第 14 天和后续每满 100 天的里程碑只出现一次。
- 结算页中第一次没写稳的字可进入专项练习；「存为字帖」能生成 PNG 并调起系统分享面板。
- 关闭再打开 App 后，`localStorage` 里的记忆模型、复习调度、个人字卡、拍字记录和跳过状态仍在。
- 飞行模式下首页和已打包笔画资源能加载。
- 底部 Tab、弹窗、字库输入框和键盘不遮挡主要按钮。
...
「我的 -> 设置 -> 练习提醒」可开启本地通知（默认关，仅 iOS 壳内生效）。设置行是 `role="switch"` 的真实开关，视觉状态、DOM 与 `aria-checked` 保持一致；VoiceOver 朗读名称、开关状态及拒绝后的限制。开关、习惯时刻、当日已练状态和未来题面全部由 Web 侧经 `shiziNative` 桥下发（`syncReminder` / `requestReminderPermission` / `queryReminderStatus`）。原生 `shiziReminderStatus` 回调是权限唯一真值：恢复备份后即使保存值写着已授权，也必须等当前设备回调；未知权限只在用户从关切到开时请求，拒绝后保持关并显示「通知权限已关闭，请前往系统设置开启」，不会反复请求；App 每次从后台回到前台都会重新查询，因此用户在系统设置改权后无需重启。Web 先从当前到期的可练字中选择，上次没写出的字优先；没有到期字时，从写错、没写出、反复看提示或仍待巩固的字中选择。被用户标记为不自然的当前语境不再进入练习或通知，字本身仍可复习；没有已审核备选时用不泄题的拼音提示并进入内容质检。没有有效目标就不调度。原生侧收到同步先清空 `shizi.reminder.*` 待发项，再最多预约未来 7 道「语境词（拼音）」题面，当天已盖章或时刻已过则跳过今天。每条通知的 `userInfo.targetCardKey` 经 `AppDelegate` 交给 `shiziOpenReminderTarget`，页面未加载完时先排队，冷启动和后台点击都进入对应练习卡。App 前台不弹提醒横幅。

## 纸墨声音

「我的 -> 设置 -> 纸墨声音」默认开启并随 JSON 备份。全 App 只有两处发声：盖章时播放不足 300ms 的低沉闷响，描红阶段每次起笔播放更轻的纸声；其他按钮、写对/写错分支和背景均无声音。iOS 通过既有 `shiziNative` 桥发送 `sound` 消息，由 `AVAudioPlayer` 播放运行时生成的短 PCM，并把 `AVAudioSession` 设为 `.ambient` + `.mixWithOthers`，因此遵守设备静音键。浏览器/PWA 只在开关开启且真实触发时创建一个 Web AudioContext；关闭状态不会创建音频上下文。

## 数据保留说明

App 使用 `WKWebsiteDataStore.default()`，Web 侧仍使用 `localStorage` 保存记忆模型、复习调度、练习日、补帖、每日完成态、当前组快照、练习提醒设置、纸墨声音设置和无设备标识的本地漏斗。月帖所需墨迹随 memory 保存，每字只留最近一次独立书写，全局最多 96 份、约 420KiB；空间不足时先淘汰墨迹，不影响调度数据。漏斗随用户主动导出的备份回传，不使用匿名信标或其他统计网络请求。iOS 中「导出备份」调用原生分享面板，只有分享完成才更新上次备份时间；「恢复备份」调用原生 Files 文档选择器。结算日帖和 1080×1440 月帖都经原生 `sharePracticeCard` 桥调起系统分享面板，图片在 WebView 内即时生成，只含对应练习字、结果与日期，不含完整记录或设备信息。浏览器/PWA 保留 Web Share、下载和 file input fallback。正常 App 更新会保留数据；卸载 App、抹掉设备数据或手动清理 WebKit 网站数据会删除本地记录。换机和长期内测前仍建议在「我的 -> 设置 -> 数据 -> 导出备份」导出 JSON。
