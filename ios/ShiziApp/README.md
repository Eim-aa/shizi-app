...
同时它会让 WKWebView 在 App 内执行一次原生 smoke：验证 7294 个可练字、开发入口隔离、本地笔画与 1100+ 条离线字源读取、页面导航、手写、防滚动、回笔、双指透视、字库专项、永久跳过和备份恢复。拍字断言会把一张真实可解码 PNG 送入裁剪压缩流程，确认不超过 64KiB 的 WebP/JPEG 缩略图与大图可读、Vision 请求实际完成、候选必须点选、失败可回退手输、生活来源落盘、单张/总量预算以及备份恢复；这只验证技术链路，不替代真机相机、照片方向和 OCR 质量验收。PointerEvent 手写还会强制检查手指等宽、Pencil 真实压力、零位移墨点、取消/中断不落墨，以及实时、恢复与分享走同一忠实渲染路径。习字链会真实提交「点拨笔画＋用户墨迹」完整字格，验证二选一判断、1.8 秒反馈、Again 事件、原子“重盖”、两步不会写教学、稳定字键 session v3、再练队列、跳过撤销以及退出后原位续练；结算断言覆盖月度节奏、第一次没写稳的字再练入口和本地分享图片的尺寸、桥接与隐私边界。字卡断言覆盖 v2 真迹矢量重绘、3:4 / 1:1 尺寸、每日一次提示、关闭偏好、相册保存与系统分享消息，并确认旧版 v1 栅格墨迹不会带着字格进入字卡；备份断言覆盖 session v3、FSRS review log、教学说明标记、本地漏斗、补帖活动、拍字记录、跳过状态、字卡偏好和限量最近墨迹。
