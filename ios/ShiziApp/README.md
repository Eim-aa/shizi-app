...
同时它会让 WKWebView 在 App 内执行一次原生 smoke：验证 7294 个可练字、开发入口隔离、本地笔画与 1100+ 条离线字源读取、页面导航、手写、防滚动、回笔、双指透视、收字和备份恢复。拍字断言会把一张真实可解码 PNG 送入裁剪压缩流程，确认不超过 64KiB 的 WebP/JPEG 缩略图与大图可读、Vision 请求实际完成、候选必须点选、失败可回退手输、生活来源落盘、单张/总量预算以及备份恢复；这只验证技术链路，不替代真机相机、照片方向和 OCR 质量验收。PointerEvent 手写还会强制检查笔锋渲染器可用、每个触点保留宽度/速度、分享归一化不丢笔锋数据。习字链会真实提交「点拨笔画＋用户墨迹」完整字格，验证二选一判断、1.8 秒反馈、Again 事件、原子“重盖”、两步不会写教学、session v2、再练队列、五类触觉以及退出后原位续练；结算断言覆盖月度节奏、薄弱字口袋和本地分享图片的尺寸、桥接与隐私边界。字卡断言覆盖 v2 真迹矢量重绘、3:4 / 1:1 尺寸、每日一次提示、关闭偏好、相册保存与系统分享消息，并确认旧版 v1 栅格墨迹不会带着字格进入字卡；备份断言覆盖 session v2、FSRS review log、教学说明标记、本地漏斗、补帖活动、拍字记录、字卡偏好和限量最近墨迹。
...
- 在「收字」中分别拍路牌、书页和菜单：每次只显示候选，不自动收字；点选目标后详情显示「拾于生活」与可放大的照片。拍到基础库外的字时会与手动输入一致生成自定义卡，笔顺缺席仍可自行书写和判断。
...
- 首页「本月拾了 N 天」与实际练习日一致；第 14 天和后续每满 100 天的里程碑只出现一次。
- 结算页薄弱字口袋可进入专项练习；「分享这张字帖」能生成 PNG 并调起系统分享面板。
