...
DECK_SIZE = 3500
DECK_KEY = "shizi.deck.v3500.context1"
GENERATED_JSON = f"selected_{DECK_SIZE}_chars.json"
if PY_DEPS.exists():
...
def choose_3500(mm, level1_chars, hanzi_db_rows):
    ...
    candidates = []
    seen = set()
    for standard_order, ch in enumerate(level1_chars, start=1):
        ...
        rank = parse_first_int(row.get("frequency_rank"), 999999 + standard_order)
    ...
    chosen = sorted(candidates, key=lambda x: (x["standard_order"], x["frequency_rank"]))[:DECK_SIZE]
    if len(chosen) < DECK_SIZE:
        chosen_chars = {item["character"] for item in chosen}
        extras = []
        for row in hanzi_db_rows:
            ch = row.get("character", "")
            if ch in chosen_chars or ch in seen or not CJK_RE.match(ch) or ch not in mm:
                continue
            strokes = parse_first_int(row.get("stroke_count"), len(mm[ch].get("matches") or []))
            groups = resolve_groups(ch)
            if groups and sum(groups) == strokes:
                pinyin = mm[ch].get("pinyin") or [row.get("pinyin", "")]
                extras.append({
                    "character": ch,
                    "pinyin": pinyin[0] if pinyin else "",
                    "definition": row.get("definition") or mm[ch].get("definition") or "",
                    "frequency_rank": parse_first_int(row.get("frequency_rank"), 999999),
                    "standard_order": 999999,
                    "stroke_count": strokes,
                    "groups": groups,
                    "decomposition": mm[ch].get("decomposition", ""),
                    "radical": mm[ch].get("radical", row.get("radical", "")),
        ...
        extras.sort(key=lambda x: (x["frequency_rank"], x["score"]))
        chosen.extend(extras[:DECK_SIZE - len(chosen)])
...
def choose_context(item, word_index):
    ...
    for word_item in word_index.get(character, []):
        ...
        score = (-(commonness_signal + 0.8 * frequency_signal) + proper_penalty + length_penalty + edge_penalty, -word_item["freq"], word)
        candidates.append((score, word, idx, py, word_item["freq"], tag, commonness))
    if not candidates:
        fallback = f"{character}字" if character != "字" else "汉字"
        return build_context(fallback)
...
def patch_index(chosen, word_index):
    ...
    for item in chosen:
        ...
        seed.append({
            "py": context["py"],
            "hint": f"{level} · 常用词语 · {item['stroke_count']}画 · {len(item['groups'])}个部件 · 字频#{item['frequency_rank'] if item['frequency_rank'] < 999999 else '未收录'}",
            "ans": context["word"],
            "ci": context["ci"],
    ...
    cards_block = (
        ...
        "  const meta={rank:w.rank||999999, strokes:w.strokes||0, parts:w.parts||1, topic:w.topic||\"综合\", d:w.d||50, level:w.level||\"高中\", common:w.common||0};\n"
    ...
    html = re.sub(
        ...
        f"// 每个字的部件分组（自动生成：{DECK_SIZE} 个一级常用汉字，按 Make Me a Hanzi matches 递归部件分组）\nconst GROUPS=" + js_string(groups) + ";",
        html,
        count=1,
    )
    html = re.sub(
...
def main():
    ...
    chosen = choose_3500(mm, level1, rows)
