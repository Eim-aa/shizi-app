...
(async () => {
  ...
  const backupUrgency = await page.evaluate(() => { const savedActivity=cloneObj(activity), savedMeta=cloneObj(backupMeta); activity=newActivity(); activity.inheritedStreak=20; activity.inheritedTotalDays=20; activity.daily={}; activity.practiceDays=[]; backupMeta=normalizeBackupMeta(null); saveActivity(); save(BACKUP_META_KEY,backupMeta); renderMe(); const accent=getComputedStyle(backupUrgency).backgroundColor, solid=Array.from(mePanel.querySelectorAll("*")).filter(node=>{ const r=node.getBoundingClientRect(); return r.width&&r.height&&getComputedStyle(node).backgroundColor===accent; }).length, todayStamp=calendarGrid.querySelector(".todayStamp"), result={status:backupStatus.textContent,color:getComputedStyle(backupStatus).color,muted:getComputedStyle(calendarMonthStat).color,urgent:getComputedStyle(document.getElementById("backupUrgency")).display,todayBg:todayStamp?getComputedStyle(todayStamp).backgroundColor:"none",accent,solid}; activity=normalizeActivity(savedActivity); backupMeta=normalizeBackupMeta(savedMeta); saveActivity(); save(BACKUP_META_KEY,backupMeta); renderMe(); return result; });
